...
class HuangdaoyushengPipeline:
    ...
    def open_spider(self,spider):
        self.file = open('hdys.txt','w',encoding='utf-8')
    ...
    def process_item(self, item, spider):
        ...
        chapter_page =re.findall(r'第(.*)',chapter_page)[0]
        item['num'] = self.change2num(chapter)  + int(chapter_page.split('/')[0])*0.1
    ...
    def close_spider(self,spider):
        ...
        for item in list_sorted:
            self.file.write("----------------%d------------------ %s--------------\n" %(item['num'],item['title']))
            self.file.write(''.join(item['content'].replace('\xa0','').replace('提示♂浏览器♂搜索♂\书名♂.\{完♂本♂神♂立占\}♂可以\快速\找\到\你\看\的\书','')) + '\n')
        self.file.close()
