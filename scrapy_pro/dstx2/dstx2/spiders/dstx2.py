...
from ..items import Dstx2Item

class Dstx2(scrapy.Spider):
    name = "dstx2"
    allowed_doamins = ['www.banzhu22.org']
    start_urls = ["http://www.banzhu22.org/2_2194/"]
    ...
    def GetContent(self,response):
        ...
        item['content'] = bs2.find('div',id='content').text.replace('\xa0','').replace('\u273f','').replace('\u01f9','').replace('\ue817','')
        yield  item
        #print(item['num'])